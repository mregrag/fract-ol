...
/*   Updated: 2024/02/11 02:43:22 by mregrag          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static	void	check_burning(t_fractal *fractal, char **argv, int argc)
{
	if (argc > 2)
		ft_error("Invalid burning, Too many argument");
	else
		fractal->name = argv[1];
}
...
{
	if (argc < 2 || argc > 4)
		ft_error("Invalid Fractol");
	if (!ft_strncmp(argv[1], "julia", 6))
		check_julia(fractal, argv, argc);
	else if (!ft_strncmp(argv[1], "mandelbrot", 11))
		check_mandelbrot(fractal, argv, argc);
	else if (!ft_strncmp(argv[1], "burning", 8))
		check_burning(fractal, argv, argc);
	else
		ft_error("Avialable fractol: mandelbrot, julia, burning");
}
